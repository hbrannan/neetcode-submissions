class Solution:
    ...
    def decode(self, s: str) -> List[str]:
        ...
        while i < len(s):
            char = s[i]
            # Assume @ delimiter; determine len
            num_digits = 0
            j = i
            ...
            # Output substr: after deliminter to end of len
            start = i + num_digits + len(self.delimiter)
            end = start + int(s[i:j]) # Len of the following str
            out.append(s[start : end])
            # Set i to next delimiter
            i = end
